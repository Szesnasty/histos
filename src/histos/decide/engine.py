...
The engine reads only the :class:`~histos.policy.contracts.GateRequest` and the
static :class:`~histos.policy.contracts.Policy` — never conversation, documents, or
...
from histos.decide import postgate as _postgate
from histos.decide.budget import (
...
from histos.policy.contracts import Effect, GateDecision, GateRequest, Policy
...
class Engine:
    ...
    def post(self, req: GateRequest, result: Any) -> tuple[GateDecision, Any]:
        """The POST chain for a tool that returned. See :mod:`histos.decide.postgate`."""
    ...
    def post_exception(self, req: GateRequest, exc: BaseException, *, mutate: bool = True) -> tuple[GateDecision, str]:
        """The POST chain for a tool that raised. See :mod:`histos.decide.postgate`."""
