...
| `pattern` | string | no | Compiled at load, so an invalid regex fails loudly — and screened at load for runaway backtracking, so a pattern imported from a third-party schema cannot hang the process that enforces it. An engine that accepts a pattern this one refuses is not compatible: the refusal is part of the format. The screen is deliberately conservative and refuses some safe shapes; the error names the rewrite. |
