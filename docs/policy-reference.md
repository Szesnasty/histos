...
| `item_type` | `string` \| `integer` \| `number` \| `boolean` \| `object` \| `any` | no | Element type for an array. String elements use declared length bounds; an element evaluated by `pattern` is additionally capped at 4,096 characters. Unpatterned text is bounded only by declared lengths and the host's aggregate input scan budget. |
...
| `max_length` | integer | no | Maximum string length, inclusive. The cheapest bound there is on an argument a manipulated model controls. With no declared maximum, unpatterned text may use the host's aggregate input scan budget; `pattern` inputs retain a separate 4,096-character safety cap. |
